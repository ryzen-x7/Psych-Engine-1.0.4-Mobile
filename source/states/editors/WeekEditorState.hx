...
class WeekEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED mobile.Utils.exists(Paths.modsImages('menubackgrounds/menu_' + assetName)) || #end
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED mobile.Utils.exists(Paths.modsImages('storymenu/' + assetName)) || #end
	...
	private static function onLoadComplete(_):Void
	{
		...
		if(fullPath != null) {
			var rawJson:String = mobile.Utils.getContent(fullPath);
			if(rawJson != null) {
